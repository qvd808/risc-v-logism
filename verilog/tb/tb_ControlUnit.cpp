...
void test_instruction(TestBench<VControlUnit> &tb, const char *name,
                      ...
                      uint8_t exp_Branch, uint8_t exp_Jump, uint8_t exp_ALUOps,
                      uint8_t expected_halt) {
  ...
  failures += tb.check("ALUOps  ", tb.dut->halt_detected, expected_halt);
...
int main(int argc, char **argv) {
  ...
  test_instruction(tb, "sub x1,x2,x3", 0x403100B3, 1, 0, 0, 0, 0, 0, 0, 0b10,
                   0);
  test_instruction(tb, "lw x1,0(x2)", 0x00012083, 1, 0, 1, 1, 1, 0, 0, 0b00, 0);
  test_instruction(tb, "sw x1,0(x2)", 0x00112023, 0, 1, 0, 0, 1, 0, 0, 0b00, 0);
  test_instruction(tb, "beq x1,x2,0", 0x00208063, 0, 0, 0, 0, 0, 1, 0, 0b01, 0);
  test_instruction(tb, "bne x1,x2,0", 0x00209063, 0, 0, 0, 0, 0, 1, 0, 0b01, 0);

  // JAL uses 0 for ALUOps as per your "keep it simple" decision
  test_instruction(tb, "jal x1,0", 0x000000EF, 1, 0, 0, 0, 1, 0, 1, 0b00, 0);

  test_instruction(tb, "and x1,x2,x3", 0x0031F0B3, 1, 0, 0, 0, 0, 0, 0, 0b10,
                   0);
  test_instruction(tb, "or x1,x2,x3", 0x0031E0B3, 1, 0, 0, 0, 0, 0, 0, 0b10, 0);
  test_instruction(tb, "xor x1,x2,x3", 0x0031C0B3, 1, 0, 0, 0, 0, 0, 0, 0b10,
                   0);
  test_instruction(tb, "Illegal instruction", 0x0, 0, 0, 0, 0, 0, 0, 0, 0, 1);
